...
export const reducer = createReducer(
  ...
  on(UserActions.cleanUserSuccess,
    (state) => {
      return {...state, registrationSuccess : undefined }
    }
  ),
  on(UserActions.updateUserPasswordSuccess,
    (state, action) => {
      return {...state, passwordUpdateSuccess : action.success }
    }
  ),
  on(UserActions.cleanUpdateUserPasswordSuccess,
    (state) => {
  ...
  on(UserActions.cleanLoginUserSuccess,
    (state) => {
      return {...state, loginSuccess : undefined }
    }
  ),
  on(UserActions.changeUserPasswordRequestSuccess,
    (state, action) => {
      return {...state, requestPasswordChangeSuccess : action.success }
    }
  ),
  on(UserActions.cleanChangeUserPasswordRequestSuccess,
    (state) => {
