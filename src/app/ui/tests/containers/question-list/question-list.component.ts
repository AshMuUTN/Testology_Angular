...
export class QuestionListComponent implements OnInit {
  ...
  loadQuestionsAndListenToStore() {
    ...
    this.store$
      .select(questionSelectors.selectQuestions)
      .pipe(
  ...
  handleLoadSuccessOrError(success) {
    ...
    if (!success) {
      const message: MessagePageParams = this.failedToLoadQuestionsParams;
  ...
  showQuestion(question: Question) {
    if (!this.subtest.isScorable) {
      const messageParams = this.notScorableMessageParams;
  ...
  get failedToLoadQuestionsParams(): MessagePageParams {
  ...
  get notScorableMessageParams(): MessagePageParams {
    const redirectUrl = this.router.url;
    return {
      text: `No se puede asignar puntaje a esta pregunta. Actualmente forma parte del subtest: ${this.subtest.name} que está configurado como un subtest de datos que no recibe puntaje. Editá la configuración del subtest para poder asignar puntaje a la pregunta.`,
      title: "Sin Puntaje",
      buttonText: "Volver",
      redirectUrl: redirectUrl,
    };
